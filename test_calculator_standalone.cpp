...
int main()
{
    MahjongCalculatorStandalone calculator;
    ...
    for (const auto& test : testCases) {
        std::cout << "輸入: " << test.input << std::endl;
        std::cout << "說明: " << test.description << std::endl;
        
        // 解析手牌
        std::vector<int> tiles = calculator.parseHandTiles(test.input);
        std::cout << "解析到 " << tiles.size() << " 張牌" << std::endl;
        
        // 計算聽牌
        std::vector<std::string> waitingTiles = calculator.calculateWaitingTiles(test.input);
        
        if (waitingTiles.empty()) {
            std::cout << "結果: 無法聽牌或格式錯誤" << std::endl;
        } else {
            std::cout << "聽 " << waitingTiles.size() << " 張牌: ";
            for (const std::string& tile : waitingTiles) {
                std::cout << tile << " ";
            }
            std::cout << std::endl;
        }
    ...
    while (true) {
        ...
        std::vector<int> tiles = calculator.parseHandTiles(input);
        std::cout << "解析到 " << tiles.size() << " 張牌" << std::endl;
        
        std::vector<std::string> waitingTiles = calculator.calculateWaitingTiles(input);
